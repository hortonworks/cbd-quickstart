...
aws_prepare_os_user() {
    ...
    cp -rf /home/ec2-user/.bash* /home/$OS_USER/
    cp /home/ec2-user/.ssh/authorized_keys /home/$OS_USER/.ssh/
    chown -Rf $OS_USER:$OS_USER /home/$OS_USER/.ssh
    service sshd reload
    echo "# This is generated by the Cloudbreak deployment's cloud-init script" >> /etc/sudoers.d/"$OS_USER"
    echo "$OS_USER ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers.d/"$OS_USER"
}
